...
def get_model():
    bert_model1 = load_trained_model_from_checkpoint(config_path, checkpoint_path)
    bert_model2 = load_trained_model_from_checkpoint(config_path, checkpoint_path)
    bert_model3 = load_trained_model_from_checkpoint(config_path, checkpoint_path)
    bert_mdoel4 = load_trained_model_from_checkpoint(config_path, checkpoint_path)

    for bert_model_ in [bert_model1,bert_model2,bert_model3,bert_mdoel4]:
        for l in bert_model_.layers:
            l.trainable = True
    ...
    B3 = bert_model3([T1, T4])
    B4 = bert_mdoel4([T2, T3])

    B1 = Lambda(lambda x: x[:, 0])(B1)
    B2 = Lambda(lambda x: x[:, 0])(B2)
    B3 = Lambda(lambda x: x[:, 0])(B3)
    B4 = Lambda(lambda x: x[:, 0])(B4)

    R = concatenate([B1, B2, B3, B4], axis=1)
