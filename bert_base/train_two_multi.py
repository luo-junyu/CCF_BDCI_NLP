...
class Evaluate(Callback):
    ...
    def on_batch_begin(self, batch, logs=None):
        ...
        if self.passed < self.params['steps']:
            ...
            K.set_value(self.model.optimizer.lr, self.lr)
            self.passed += 1
        elif self.passed > self.params['steps'] * 10:
            self.lr += min_learning_rate / 5
            K.set_value(self.model.optimizer.lr, self.lr)
            self.passed += 1
...
oof_test /= 5
np.savetxt(model_save_path+'train_bert.txt', oof_train)
np.savetxt(model_save_path+'test_bert.txt', oof_test)
